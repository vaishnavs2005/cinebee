...
export class Conversion {
    ...
    async _processVideoTrack(track, trackOptions, outputTrackId) {
        ...
        const needsTranscode = !!trackOptions.forceTranscode
            || (trackOptions.forceTranscode !== false && firstTimestamp < this._startTimestamp)
        ...
        if (!needsTranscode) {
            ...
            this._registerTrackPump(async (pump) => {
                ...
                const startPacket = this._startTimestamp > 0 ? await sink.getKeyPacket(this._startTimestamp) : undefined;
                const effectiveStart = startPacket ? startPacket.timestamp : this._startTimestamp;
                for await (const packet of sink.packets(startPacket, undefined, { verifyKeyPackets: true })) {
                    ...
                    const modifiedPacket = packet.clone({
                        timestamp: packet.timestamp - effectiveStart,
